    ...
    try:
        ...
        if gps_info:
            ...
            if "GPSLatitude" in gps_tag_map and "GPSLatitudeRef" in gps_tag_map:
                lat = _convert_to_degrees(gps_tag_map["GPSLatitude"])
                if lat is not None and gps_tag_map["GPSLatitudeRef"] in ["S", b"S"]:
                    lat = -lat

            if "GPSLongitude" in gps_tag_map and "GPSLongitudeRef" in gps_tag_map:
                lon = _convert_to_degrees(gps_tag_map["GPSLongitude"])
                if lon is not None and gps_tag_map["GPSLongitudeRef"] in ["W", b"W"]:
                    lon = -lon
