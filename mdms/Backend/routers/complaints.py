...
async def upload_batch_complaints(
    ...
    for file in files:
        ...
        # ---------- IMAGE GPS (Disabled as per user request) ----------
        ...
        # Fallback to manual coordinates if EXIF is missing
            ...
            if issue_type is None or issue_type not in AUTHORITY_MAP:
                ...
                for item in items:
                    ticket_result["rejected_items"].append({
                        ...
                        "message": f"No valid issue detected in this {item['media_type']}.",
            ...
            for item in items:
                ...
                if item.get("no_detection"):
                    ...
                    rejected_items.append({
                        "file_name": item["file_name"],
                        "media_type": item["media_type"],
                        "message": f"No detection found in the {item['media_type']}.",
                        "latitude": item.get("latitude"),
                        "longitude": item.get("longitude"),
                        "gps_extracted": item.get("gps_extracted"),
                    })
