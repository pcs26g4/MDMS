...
# Full-featured detect.py with webcam, video, image support
...
def run(
    ...
    # Check source type
    webcam = source.isnumeric() or source.endswith('.txt') or source.startswith(('rtsp://', 'http://', 'https://'))
...
def parse_opt():
    ...
    opt.imgsz = opt.imgsz if len(opt.imgsz) > 1 else [opt.imgsz[0], opt.imgsz[0]]
    return opt
