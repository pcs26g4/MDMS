...
export default function AdminPage() {
  ...
  const updateComplaintStatus = async (id, newStatus) => {
    try {
      ...
      setErrorPopupMessage(err.message || "Failed to update complaint status");
  ...
  // Updated removeInspector function
  ...
  const deleteAccount = async (id, name) => {
    if (window.confirm(`Delete account: ${name}?\nThis action cannot be undone.`)) {
      try {
        ...
        setErrorPopupMessage(err.message || "Failed to delete account");
        setShowErrorPopup(true);
      }
    }
  };
    ...
    <div className="admin-layout">
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <button
          ...
          onClick={() => { setView("inspectors"); setSidebarOpen(false); }}
        >
          <span className="btn-icon">👮</span>
          Inspectors
          <span className="badge-count">{inspectors.length}</span>
        </button>
        ...
        <button
          ...
          onClick={() => { setView("actions"); setSidebarOpen(false); }}
        >
          <span className="btn-icon">📝</span>
          Inspector Actions
          {actions.length > 0 && (
            <span className="badge-count">{actions.length}</span>
          )}
        </button>
        ...
        >
          <span className="btn-icon">👥</span>
          All Accounts
          <span className="badge-count">{allAccounts.length}</span>
        </button>
        ...
        <div className="modal-overlay">
          <div className="modal-content success-modal-content" style={{ maxWidth: '400px', padding: '0', overflow: 'hidden' }}>
            <div className="success-modal-header" style={{ background: 'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)' }}>
              <span className="success-icon-large">🗑️</span>
            ...
            <>
              ...
              <div className="admin-card">
                <div className="card-header">
                  ...
                  <button
                    className="add-inspector-btn-small"
                    onClick={() => setShowAddInspectorModal(true)}
                    ...
                    {filteredInspectors.map(i => (
                      <div className="inspector-card" key={i.id}>
                        <div className="inspector-header">
                          ...
                          <span className="status-badge approved">INSPECTOR</span>
                        </div>
